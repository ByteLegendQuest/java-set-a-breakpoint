...
`git clone https://github.com/ByteLegendQuest/java-set-a-breakpoint`

A project directory named `java-set-a-breakpoint` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-set-a-breakpoint` directory which is created just now.
